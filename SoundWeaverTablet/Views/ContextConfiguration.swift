...
struct ContextConfiguration: View {
    ...
    /// A binding to the set of sounds the app monitors for a specific context.
    ...
    /// A list of all sounds the app displays for user to select from after the search.
    ///
    /// The first item in the option pair (i.e., SoundIdentifier) identifies the sound class to display. The second item indicates whether the user has enabled it. The app lists options in the order in which they appear in the selection UI.
    ...
    var headerContent: some View {
        VStack {
            ...
            Text("Select Sounds to Detect for this Context").font(.title).frame(alignment: .leading)
            HStack {
    ...
    var soundOptionsList: some View {
        List {
            ForEach(displayedSoundOptions, id: \.0) { classAndSelectionStatus in
                Button(action: { ContextConfiguration.toggleMembership(member: classAndSelectionStatus.0, set: &selectedSounds) }) {
                    HStack {
